...
void StochSymMatrix::copyMtxFromDouble(int copyLength,double *values)
{}

void StochSymMatrix::copyDiagonalVal_FromcopyDiagonalVal_From( int idiag, OoqpVector& v, 
							bool firstCall, std::map<int,int> &ValIdxMap )
{
	assert( "Not implemented" && 0 );
}

void StochSymMatrix::dumpToFile(std::string name, int nchild, int giterNum) {
  int m,n;
  this->getSize(m,n);
  printf("%s 1 has size %dx%d\n", name.c_str(), m, n);
  if(n==0 || m==0) {
