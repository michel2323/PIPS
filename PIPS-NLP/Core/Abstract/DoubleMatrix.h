...
public:
  ...
  virtual void copyDiagonalVal_From( int idiag, OoqpVector& x, 
  				bool firstCall, std::map<int,int> &ValIdxMap ) {assert(0);};
...
public:
  ...
  virtual void copyMtxFromDouble(int copyLength,double *values){assert("not done" &&0);};

  virtual void setAdditiveDiagonal(OoqpVector& additiveDiag_){assert("not done" &&0);};

  virtual void copyDiagonalVal_From( int idiag, OoqpVector& x, 
  						bool firstCall, std::map<int,int> &ValIdxMap) {assert("not done" &&0);};
  
  virtual void fromGetSpRow_WithRowStart( int row, int col,
                             double A[], int lenA, int jcolA[], int& nnz,
                             int colExtent, int& info, int & rowStart )  {assert("not done" &&0);};  
  
  virtual void fromGetDense_withMap( int row, int col, double * A, int lda,
			     int rowExtent, int colExtent, int const FirstCall, std::map<int,int> &ValIdxMap ) {assert("not done" &&0);};   

  virtual double * getMatVal(){assert("not done" &&0); return NULL;};
